...
M.ns = vim.api.nvim_create_namespace('fk_markdown.nvim')
...
function M.get(buf)
    ...
    if not result then
        result = require('fk_markdown.lib.decorator').new(buf)
        M.cache[buf] = result
    end
    return result
end
    ...
    if ok and parser then
        ...
        if context then
            ...
            context.view:parse(parser, function()
                local Extmark = require('fk_markdown.lib.extmark')
                local handlers = require('fk_markdown.core.handlers')
