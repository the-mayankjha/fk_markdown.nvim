local Line = require('fk_markdown.lib.line')
local Resolved = require('fk_markdown.lib.resolved')
...
function Config.new(root, enabled, buf, custom)
    ...
    local src = require('fk_markdown.core.preview').get(buf)
...
function Config.schema(child)
    ...
    local parent = {
        ...
        plant_uml = settings.plant_uml.schema(),
        image = settings.image.schema(),
