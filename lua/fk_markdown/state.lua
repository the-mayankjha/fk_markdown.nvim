local Config = require('fk_markdown.lib.config')
...
function M.setup(config)
    ...
    require('fk_markdown.core.ts').setup()
    require('fk_markdown.core.ui').setup()
end
...
function M.attach()
    require('fk_markdown.core.ts').init()
    if M.completions.lsp.enabled then
        require('fk_markdown.integ.lsp').init()
    elseif M.completions.blink.enabled then
        require('fk_markdown.integ.blink').init()
    elseif M.completions.coq.enabled then
        require('fk_markdown.integ.coq').init()
    else
        require('fk_markdown.integ.cmp').init()
    end
end
...
function M.difference()
    local default = require('fk_markdown').default
    return require('fk_markdown.debug.diff').get(default, M.config)
end

---@return string[]
function M.validate()
    local settings = require('fk_markdown.settings')
    return require('fk_markdown.debug.schema').validate(
        M.config,
        Config.schema({
